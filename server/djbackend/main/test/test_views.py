...
class TestStreamView(TestCase):
    ...
    def test_proper_camera_queryset(self):
        self.client.login(username=self.test_user, password=self.test_password)
        response = self.client.get(reverse('stream'))
        expected_camera_name = Camera.objects.get(
            camera_name=self.active_camera_id
        )
        cam_list = [expected_camera_name.camera_name]
        expected_cam_list = json.dumps(cam_list)
        self.assertEqual(response.status_code, 200)
        self.assertEqual(str(response.context['user']), self.test_user)
        self.assertEqual(response.context['camera_list'][0].camera_name,
...
class TestArchiveView(TestCase):
    ...
    def setUp(cls):
        ...
        cls.test_video_pk = ArchiveVideo.objects.create(
            date_created=cls.current_date - datetime.timedelta(days=10),
            human_det=False,
            chiken_det=False,
            cat_det=False,
            car_det=False,
    ...
    def test_get_item_by_date(self):
        self.client.login(username=self.test_user, password=self.test_password)
        response = self.client.get(f'/archive?date_created={self.date_string}',
                                   follow=True)
        self.assertEqual(str(response.context['user']), self.test_user)
        self.assertEqual(response.status_code, 200)
    ...
    def test_filter_by_camera(self):
        self.client.login(username=self.test_user, password=self.test_password)
        response = self.client.get(f'/archive?camera_id={self.test_camera}',
                                   follow=True)
        self.assertEqual(str(response.context['user']), self.test_user)
        self.assertEqual(response.status_code, 200)
        self.assertTrue('page_obj' in response.context)
    ...
    def test_no_video_with_new_camera(self):
        self.client.login(username=self.test_user, password=self.test_password)
        response = self.client.get(f'/archive?camera_id={self.wrong_camera}',
                                   follow=True)
        self.assertEqual(str(response.context['user']), self.test_user)
        self.assertEqual(response.status_code, 200)
        self.assertTrue('page_obj' in response.context)
    ...
    def test_full_query(self):
        self.client.login(username=self.test_user, password=self.test_password)
        response = self.client.get(
            f'/archive?camera_id={self.test_camera}\
            &cat_det=True\
            &car_det=True\
            &chiken_det=True\
            &human_det=True\
            &date_created={self.date_string}',
            follow=True)
        self.assertEqual(str(response.context['user']), self.test_user)
        self.assertEqual(response.status_code, 200)
        self.assertTrue('page_obj' in response.context)
...
class TestVideoDetailView(TestCase):
    ...
    def setUp(cls):
        ...
        test_user = User.objects.create_user(username=cls.test_user,
                                             ...
                                             is_active=True,)
        test_user.save()
        cls.test_camera_pk = Camera.objects.create(
            camera_name='test',
            is_active=True,
        ).pk
        cls.request = {
            'request_type': 'video_request',
            'video_name': (cls.video_name + '|' + cls.test_camera_pk)
        }
    ...
    def test_get_context_data_request_video_with_cache(self):
        self.test_cached_video_pk = CachedVideo.objects.create(
            name=self.video_name,
            date_expire=self.current_date + datetime.timedelta(days=10))
    ...
    def test_call_update_cache(self):
        ...
        with patch('main.views.update_cache') as update_cache:
            self.client.login(username=self.test_user,
                              password=self.test_password)
            self.client.get(f'/archive/{self.test_video_pk}')
    ...
    def test_GET_anonymous_user(self):
        with patch('main.views.VideoDetailView.request_video') as request:
            ...
            self.assertRedirects(
                response,
                f'/accounts/login/?next=/archive/{self.test_video_pk}'
            )
