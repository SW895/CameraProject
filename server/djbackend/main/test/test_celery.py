...
class TestCleanExpiredRecords(TestCase):
    ...
    def setUpTestData(cls):
        cls.expire_id = CachedVideo.objects.create(
            name='expired_record',
            date_expire=datetime.datetime.now(tz=timezone)
            - datetime.timedelta(days=100)
        ).pk
        cls.actual_id = CachedVideo.objects.create(
            name='actual_record',
            date_expire=datetime.datetime.now(tz=timezone)
            + datetime.timedelta(days=100)
        ).pk

        cls.expired_name = str(settings.MEDIA_ROOT)
        + '/'
        + 'expired_record'
        + '.mp4'
        ...
        cls.actual_name = str(settings.MEDIA_ROOT)
        + '/'
        + 'actual_record'
        + '.mp4'
    ...
    def test_expired_record_delete(self):
        clean_expired_records.apply().get()
        self.assertFalse(
            CachedVideo.objects.filter(id=self.expire_id).exists()
        )

    def test_expired_record_file_delete(self):
        clean_expired_records.apply().get()
        self.assertFalse(
            os.path.exists(self.expired_name)
        )

    def test_actual_record_no_delete(self):
        clean_expired_records.apply().get()
        self.assertTrue(
            CachedVideo.objects.filter(id=self.actual_id).exists()
        )

    def test_actual_file_no_delete(self):
        clean_expired_records.apply().get()
        self.assertTrue(
            os.path.exists(self.actual_name)
        )
...
class TestCleanVideo(TestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.lost_name = str(settings.MEDIA_ROOT)
        + '/'
        + 'lost_video'
        + '.mp4'
...
class TestUpdateCache(TestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.cache_pk = CachedVideo.objects.create(
            name=cls.video_name,
            date_expire=cls.date_expire
        ).pk

    def test_cache_update_correctly(self):
        update_cache.apply(args=(self.video_name, self.timeout)).get()
        ...
        self.assertEqual(
            self.date_expire.second
            + datetime.timedelta(seconds=self.timeout).seconds,
